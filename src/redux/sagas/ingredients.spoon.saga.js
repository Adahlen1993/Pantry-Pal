...
function* fetchIngredientsSpoon(action) {
  try {
    const response = yield axios.get("/api/ingredients/spoon", action.payload);
    yield put({ type: "SET_INGREDIENTS_SPOON", payload: response.data });
  } catch (error) {
